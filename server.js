...
io.on('connection', (socket) => {
	...
	/* check that client sent data is good */ 
	socket.on('join_room', (payload) => {
		serverLog('Server received a command', '\'join room\'', JSON.stringify(payload));
	...
	socket.on('invite', (payload) => {
        ...
        // make sure data from client is good 
        if ((typeof payload == 'undefined') || (payload === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'client did not send a payload';
    	...
    	// make sure that the invited player is here
    ...
    socket.on('uninvite', (payload) => {
        ...
        //make sure invited player is here
        io.in(room).allSockets().then((sockets) => {
	...
	socket.on('game_start', (payload) => {
	    serverLog('Server received a command', '\'uninvite\'', JSON.stringify(payload));
	    //check data from client is good
	    if ((typeof payload == 'undefined') || (payload === null)) {
	        response = {};
	        response.result = 'fail';
	        response.message = 'client did not send a payload';
	...
	socket.on('send_chat_message', (payload) => {
		...
		if ((typeof username == 'undefined') || (username === null)){
			response = {};
			response.result = 'fail';
			response.message = 'client did not send a valid username for message';
			socket.emit('send_chat_message_response', response);
			serverLog('join_room command failed', JSON.stringify(response));
			return;
		}
	...
	socket.on('play_token', (payload) => {
        ...
        //make sure turn color is correct 
        ...
        // make sure current player is correct 
        if (
...
function check_line_match(color, dr, dc, r, c, board) {
    ...
    // check to see if we will exceed board boundary
    if ((r + dr < 0 ) || (r +dr > 7)) {
        return false;
    }
...
function send_game_update(socket, game_id, message) {
    ...
    // check that only 2 players to a room
    ...
    if (legal_moves === 0) {
        ...
        let winner = "Tie Game. Dal Mi has to choose.";
        if (whitesum > blacksum) {
            winner = "Han Ji Pyeong (white)";
        }
        if (whitesum < blacksum) {
            winner = "Nam Do San (black)";
        }
