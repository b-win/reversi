...
let username = decodeURI(getIRIParamVal('username'));
...
let chatRoom = decodeURI(getIRIParamVal('game_id'));
if ((typeof chatRoom == 'undefined') || (chatRoom === null) || (chatRoom === 'null')) {
    chatRoom = "Lobby";
}

/* set up the socket.io connection to server */
...
socket.on('join_room_response', (payload) => {
    ...
    let newHTML = '<p class=\'join_room_response\'>' + payload.username + ' joined the chat. (There are ' + payload.count + ' users in this room.)</p>';
    let newNode = $(newHTML);
    newNode.hide();
...
socket.on('game_over', (payload) => {
    ...
    // end game, go back to lobby pop up
